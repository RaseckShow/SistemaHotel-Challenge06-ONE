...
public class ReservasView extends JFrame {
	...
	public JComboBox<Format> txtFormaPago;
	int xMouse, yMouse;
	private JLabel labelExit;
	private JLabel lblValorSimbolo; 
	private JLabel labelAtras;
	private ReservacionController reservacionController = new ReservacionController();
...
//Activa el evento, después del usuario seleccionar las fechas se debe calcular el valor de la reserva
			if (txtFechaE.getDate() != null && txtFechaS.getDate() !=null) {			
				String txtFechaEntrada = ((JTextField)txtFechaE.getDateEditor().getUiComponent()).getText();
				String txtFechaSalida =  ((JTextField)txtFechaS.getDateEditor().getUiComponent()).getText();
		
				String cantidadString = Integer.toString(calcularPrecio(txtFechaEntrada, txtFechaSalida));
				txtValor.setText(cantidadString);
			}
			}
		});
		...
		txtValor.setBounds(150, 328, 80, 33);
		...
		JLabel lblValor = new JLabel("CANTIDAD A PAGAR");
	...
	String txtFechaEntrada = ((JTextField)txtFechaE.getDateEditor().getUiComponent()).getText();
	String txtFechaSalida =  ((JTextField)txtFechaS.getDateEditor().getUiComponent()).getText();
	Integer txtPrecio = calcularPrecio(txtFechaEntrada, txtFechaSalida); 
	if (txtPrecio == 0) {
		return ;
	}

	String txtFormaPagoElegido = (String) txtFormaPago.getSelectedItem();
	...
	private Integer calcularPrecio(String txtFechaEntrada, String txtFechaSalida){
		SimpleDateFormat formatDate = new SimpleDateFormat("yyyy-MM-dd");
		int precioXNoche = 200;
		try {
			Date dataFormateadaEntrada = formatDate.parse(txtFechaEntrada);
			Date dataFormateadaSalida = formatDate.parse(txtFechaSalida);

			int milisecondsByDay = 86400000;
				if (dataFormateadaEntrada.getTime() > dataFormateadaSalida.getTime() ) {
					JOptionPane.showMessageDialog(null, "Fecha de Salida Incorrecta");
					return 0;
				}
				int nochesAHospedarse = (int) ((dataFormateadaSalida.getTime() - dataFormateadaEntrada.getTime()) / milisecondsByDay);
				Integer cantidadAPagar = nochesAHospedarse * precioXNoche;
